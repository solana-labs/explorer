...
export function BlockHistoryCard({ slot, blockhash }: { slot: string, blockhash: string }) {
  const block = useBlock(slot ? slot : blockhash);
  const fetchBlock = useFetchBlock();
  const refresh = () => fetchBlock(slot ? slot : blockhash, slot ? 'slot' : 'blockhash');
      ...
      <TableCardBody>
        {
          block ?
            block.data?.Tags ?
            block.data?.Tags
            .filter((tag: any) => tag.name === 'slot' || tag.name === 'parentSlot' || tag.name === 'blockhash')
            .map((tag: any) => {
              return(
                <tr>
                  <td>{tag.name}</td>
            ...
            : invalidOverview() : invalidOverview()
        }
      </TableCardBody>
    </div>
