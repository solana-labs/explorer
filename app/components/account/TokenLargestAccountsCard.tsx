...
export function TokenLargestAccountsCard({ mintAddress }: { mintAddress: string }) {
    ...
    if (largestAccounts?.data === undefined) {
        ...
        return <ErrorCard retry={refreshLargest} text="Failed to fetch largest accounts" />;
...
const renderAccountRow = (
    ...
    if (supply > 0 && account.uiAmountString) {
        const uiAmountPercent = new BigNumber(account.uiAmountString).times(100).dividedBy(supply);
    ...
    return (
        <tr key={index}>
            ...
            <td>{account.owner && <Address pubkey={account.owner} link truncate />}</td>
